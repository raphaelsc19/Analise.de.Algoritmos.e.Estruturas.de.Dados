...
int transformapalavra (char* palavra){
    //ASCII table
}
...
struct tNo * criaNo(char* palavra)
{
    int i, chave;
    chave = transformapalavra (palavra);
    struct tNo *it = (struct tNo*) malloc(sizeof(struct tNo));
    if(it!=NULL)
    {
        strcpy (palavra, it -> palavra);
        it->chave = chave;
...
struct tNo * remover(struct tArvore *t, int k)
{
